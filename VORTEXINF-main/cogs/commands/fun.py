...
class Fun (commands .Cog ):
  ...
  async def compliment (self ,ctx ,user :discord .User =None ):
      if user is None:
          user = ctx.Developer: NEVA "3pa8"_types = [
  ...
  async def image (self ,ctx ,*,search_query :str ):
        if not ctx .channel .is_nsfw ():
            view = ui.LayoutView()
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] This command can only be used in NSFW (age-restricted) channels."))
            view.add_item(container)
        ...
        async with aiohttp .ClientSession ()as session :
            async with session .get (f"https://www.googleapis.com/customsearch/v1?key={self.google_api_key}&cx={self.search_engine_id}&q={search_query}&searchType=image")as response :
                ...
                if "items"in data :
                    ...
                    view = ui.LayoutView()
                    container = ui.Container(accent_color=None)
                    container.add_item(ui.TextDisplay("[x] No images found for that search query."))
                    view.add_item(container)
                    await ctx.reply(view=view)
      ...
      async with aiohttp .ClientSession ()as session :
          async with session .get (f"https://api.giphy.com/v1/gifs/search?api_key={self.giphy_token}&q={search_query}&limit=10")as response :
              ...
              if "data"in data :
                  ...
                  view = ui.LayoutView()
                  container = ui.Container(accent_color=None)
                  container.add_item(ui.TextDisplay("[x] No GIFs found for that search query."))
                  view.add_item(container)
                  await ctx.reply(view=view)
      ...
      try :
        async with session .get (f"http://ip-api.com/json/{ip}")as response :
          ...
          if data ['status']=='fail':
            view = ui.LayoutView()
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] Failed to retrieve data. Please check the IP address and try again."))
            view.add_item(container)
            await ctx.reply(view=view)
            return 
      ...
      async with aiohttp .ClientSession ()as session :
          async with session .get (complete_url )as response :
              ...
              if data ["cod"]!="404":
                  ...
                  view = ui.LayoutView()
                  container = ui.Container(accent_color=None)
                  container.add_item(ui.TextDisplay("[x] City not found. Please enter a valid city name."))
                  view.add_item(container)
                  await ctx.reply(view=view)
  ...
  async def fake_ban (self ,ctx ,user :discord .Member ):
    ...
    container.add_item(ui.Separator())
    container.add_item(ui.TextDisplay(f"[check] [{user.display_name}](https://discord.com/users/{user.id}) has been successfully banned"))
    container.add_item(ui.Separator())
  ...
  async def eight_ball (self ,ctx ,*,question :str =None ):
      if question is None :
          view = ui.LayoutView()
          container = ui.Container(accent_color=None)
          container.add_item(ui.TextDisplay("[x] You need to ask a question!"))
          view.add_item(container)
          await ctx.reply(view=view)
          return 
          ...
          async with session .get ("https://api.truthordarebot.xyz/api/truth")as response :
              if response .status ==200 :
                  ...
                  if question :
                      ...
                      view = ui.LayoutView()
                      container = ui.Container(accent_color=None)
                      container.add_item(ui.TextDisplay("[x] Couldn't retrieve a truth question. Please try again."))
                      view.add_item(container)
                      await ctx.reply(view=view)
              else :
                  pass 
          ...
          async with session .get ("https://api.truthordarebot.xyz/api/dare")as response :
              if response .status ==200 :
                  ...
                  if question :
                      ...
                      view = ui.LayoutView()
                      container = ui.Container(accent_color=None)
                      container.add_item(ui.TextDisplay("[x] Couldn't retrieve a dare question. Please try again."))
                      view.add_item(container)
                      await ctx.reply(view=view)
              else :
                  pass 
  ...
  async def translate_command (self ,ctx ,*,message =None ):
      ...
      if ctx .message .reference :
        try :
          ...
          if replied_msg and replied_msg .content :
            ...
            view = ui.LayoutView()
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] No content found in the replied message."))
            view.add_item(container)
            await ctx.reply(view=view)
            return 
        except :
          view = ui.LayoutView()
          container = ui.Container(accent_color=None)
          container.add_item(ui.TextDisplay("[x] Could not fetch the replied message."))
          view.add_item(container)
          await ctx.reply(view=view)
          return 
      else :
        view = ui.LayoutView()
        container = ui.Container(accent_color=None)
        container.add_item(ui.TextDisplay("[x] Please provide text to translate or reply to a message."))
        view.add_item(container)
        await ctx.reply(view=view)
        return 
    else :
    ...
    if not text_to_translate or not text_to_translate .strip ():
      view = ui.LayoutView()
      container = ui.Container(accent_color=None)
      container.add_item(ui.TextDisplay("[x] No text provided for translation."))
      view.add_item(container)
      await ctx.reply(view=view)
      return 
    ...
    try :
      ...
      if not groq_api_key :
        ...
        view = ui.LayoutView()
        container = ui.Container(accent_color=None)
        container.add_item(ui.TextDisplay("[x] Translation service is not configured."))
        view.add_item(container)
        await ctx.reply(view=view)
        return 
