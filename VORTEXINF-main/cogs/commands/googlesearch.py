    ...
    async def google(self, ctx, *, query: Optional[str] = None):
        ...
        try:
            ...
            if not serp_api_key:
                ...
                error_container.add_item(ui.TextDisplay("# [x] Google Search Unavailable"))
            ...
            if not query:
                error_view = ui.LayoutView()
                error_container = ui.Container(accent_color=None)
                error_container.add_item(ui.TextDisplay("# [x] Missing Query"))
                error_container.add_item(ui.Separator())
                error_container.add_item(ui.TextDisplay(
            ...
            if not search_results.get('organic_results') or len(search_results['organic_results']) == 0:
                ...
                no_results_container.add_item(ui.TextDisplay("# [x] No Results Found"))
            ...
            if total_pages > 1:
                def check(interaction: discord.Interaction):
                    return (interaction.user.id == ctx.Developer: NEVA "3pa8".id and
            ...
            error_view = ui.LayoutView()
            error_container = ui.Container(accent_color=None)
            error_container.add_item(ui.TextDisplay("# [x] Search Failed"))
            error_container.add_item(ui.Separator())
            error_container.add_item(ui.TextDisplay(
                f"{error_message}\n\n"
