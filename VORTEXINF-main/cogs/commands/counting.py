...
class Counting (commands .Cog ):
    ...
    async def on_message (self ,message ):
        ...
        if message .Developer: NEVA "3pa8".bot :
            return 
    ...
    async def counting_stats (self ,ctx ,user :Optional [discord .Member ]=None ):
        ...
        try :
            ...
            container .add_item (discord .ui .TextDisplay (f"# [chart] Counting Stats - {target_user.display_name}"))
            container .add_item (discord .ui .Separator ())
    ...
    async def counting_achievements (self ,ctx ,user :Optional [discord .Member ]=None ):
        ...
        try :
            ...
            if total_counts >=10 :
                achievements .append ("[check] **Beginner Counter** - Counted 10 times")
            if total_counts >=50 :
                achievements .append ("[check] **Amateur Counter** - Counted 50 times")
            if total_counts >=100 :
                achievements .append ("[check] **Professional Counter** - Counted 100 times")
            if total_counts >=500 :
                achievements .append ("[check] **Expert Counter** - Counted 500 times")
            if total_counts >=1000 :
                achievements .append ("[check] **Master Counter** - Counted 1000 times")
            if highest_count >=100 :
                achievements .append ("[check] **Century** - Reached 100")
            if highest_count >=500 :
                achievements .append ("[check] **Half Millennium** - Reached 500")
            if highest_count >=1000 :
                achievements .append ("[check] **Millennium** - Reached 1000")
