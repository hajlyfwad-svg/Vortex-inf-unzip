...
class UserinfoLayout(ui.LayoutView):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if interaction.user != self.ctx.Developer: NEVA "3pa8".response.send_message("You cannot interact with this menu.", ephemeral=True)
            return
...
class General(commands.Cog):
    ...
    async def _user (self ,ctx ,member :Optional [Union [discord .Member ,discord .User ]]=None ):
        try :
            ...
            container.add_item(discord.ui.TextDisplay(f"[user] **{member.display_name}'s Avatar**"))
            ...
            if user.avatar:
                ...
                container.add_item(discord.ui.TextDisplay("[x] **No Custom Avatar**\nThis user is using Discord's default avatar."))
    ...
    async def poll (self ,ctx : commands .Context ,*,message ):
        ...
        container.add_item(discord.ui.TextDisplay(f"# [chart] Poll"))
        container.add_item(discord.ui.Separator())
    ...
    async def users (self ,ctx : commands .Context ):
        ...
        container.add_item(discord.ui.TextDisplay(f"# [robot] {self.bot.user.name} Statistics"))
        container.add_item(discord.ui.Separator())
    ...
    async def wizz (self ,ctx : commands .Context ):
        ...
        container.add_item(discord.ui.TextDisplay(f"# [lightning] {self.bot.user.name} Wizz Complete"))
        container.add_item(discord.ui.Separator())
    ...
    async def rickroll (self ,ctx : commands .Context ,*,url :str ):
        ...
        try:
            ...
            status_emoji = "🚨" if rickRoll else "[check]"
            ...
            result_text = f"**URL:** {url}\n**Status:** {'[warning] **RICKROLL DETECTED**' if rickRoll else '[check] **Safe to click**'}\n**Confidence:** {'High' if rickRoll else 'N/A'}"
    ...
    async def hash (self ,ctx : commands .Context ,algorithm :str ,*,message ):
        ...
        container.add_item(discord.ui.TextDisplay(f"# [lock] Hash Generator"))
        container.add_item(discord.ui.Separator())
    ...
    async def invite (self ,ctx : commands .Context ):
        ...
        container.add_item(ui.TextDisplay(f"# [sparkles] Invite {self.bot.user.name}"))
    ...
    async def tts(self, ctx: commands.Context, voice: Optional[str] = "male", *, text: str):
        if len(text) > 500:
            await ctx.reply("[x] Text is too long. Please keep it under 500 characters.")
            return
        ...
        try:
            ...
            async with self.aiohttp.get(tts_url) as response:
                if response.status == 200:
                    ...
                    await ctx.reply("[x] Failed to generate speech. Please try again later.")
                    
        except Exception as e:
            await ctx.reply(f"[x] An error occurred: {str(e)}")
