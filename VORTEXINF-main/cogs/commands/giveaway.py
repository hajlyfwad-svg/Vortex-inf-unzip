...
async def giveaway_button_callback(interaction: discord.Interaction, giveaway_id: int):
    ...
    async with aiosqlite.connect('db/giveaways.db') as db:
        ...
        if await cursor.fetchone():
            ...
            container.add_item(ui.TextDisplay("[x] Already Entered\n\nYou've already entered this giveaway!"))
        ...
        container = ui.Container(accent_color=None)
        container.add_item(ui.TextDisplay(f"[check] Entry Confirmed!\n\nYou've successfully entered the giveaway!\n\n**Total Entries:** {count}"))
...
class Giveaway(Cog):
    ...
    async def gstart(self, ctx: Context, time: str, winners: int, *, prize: str):
        ...
        if seconds <= 0:
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] Invalid Time\n\nPlease use format: `1s`, `1m`, `1h`, or `1d`"))
        ...
        if winners < 1:
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] Invalid Winners\n\nMust have at least 1 winner!"))
        ...
        async with aiosqlite.connect('db/giveaways.db') as db:
            cursor = await db.execute(
                ...
                (ctx.guild.id if ctx.guild else 0, ctx.channel.id, ctx.Developer: NEVA "3pa8".id, prize, winners, end_time)
            )
            await db.commit()
        ...
        if giveaway_id is None:
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] Error\n\nFailed to create giveaway!"))
    ...
    async def gend(self, ctx: Context, message: discord.Message | None = None):
        ...
        if target_message is None:
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[x] No Message Found\n\nReply to a giveaway message or provide message ID!"))
        ...
        async with aiosqlite.connect('db/giveaways.db') as db:
            ...
            if not giveaway:
                container = ui.Container(accent_color=None)
                container.add_item(ui.TextDisplay("[x] Not Found\n\nNo active giveaway found for this message!"))
            ...
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("[check] Giveaway Ended\n\nThe giveaway has been ended successfully!"))
    ...
    async def greroll(self, ctx: Context, message: discord.Message | None = None):
        ...
        async with aiosqlite.connect('db/giveaways.db') as db:
            ...
            if not entries_list:
                container = ui.Container(accent_color=None)
                container.add_item(ui.TextDisplay("[x] No Entries\n\nNo entries found to reroll!"))
