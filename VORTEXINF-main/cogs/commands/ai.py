...
[chart] COMMUNITY: Leveling, leaderboards, welcome systems, tickets
[wrench] UTILITIES: Server management, logging, backup, verification
[target] AI FEATURES: Conversations, image analysis, code generation, explanations
...
Ready to have meaningful conversations and help with anything you need! 💖"""
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        ...
        try :
            ...
            container.add_item(discord.ui.TextDisplay(f"# [sparkles] Personality Set"))
            ...
            error_container = discord.ui.Container(
                discord.ui.TextDisplay(f"[x] **Error**\n\nFailed to save personality: {e}"),
                accent_color=None
            )
...
Support server: https://discord.gg/JxCFmz9nZP"""
    ...
    async def ai_conversation_stats (self ,ctx :commands .Context ):
        ...
        if not stats :
            container.add_item(discord.ui.TextDisplay(f"# [chart] Conversation Statistics"))
            container.add_item(discord.ui.Separator())
            ...
            container.add_item(discord.ui.TextDisplay(f"# [chart] Your Conversation Statistics"))
            container.add_item(discord.ui.Separator())
    ...
    async def ai_ask (self ,ctx :commands .Context ,*,question :str ):
        ...
        try :
            ...
            container.add_item(discord.ui.TextDisplay(f"# [robot] AI Response"))
            container.add_item(discord.ui.Separator())
            ...
            error_view = discord.ui.LayoutView()
            error_container = discord.ui.Container(
                discord.ui.TextDisplay(f"# [robot] AI Response\n\n[x] **Error processing question**\n\nPlease try again later."),
                accent_color=None
            )
            error_view.add_item(error_container)
    ...
    async def disable_roleplay (self ,ctx ):
        ...
        if channel_id not in self .roleplay_channels :
            ...
            await ctx .send (view=view )
            return 
    async def _get_gemini_response(self, prompt: str, history: list) -> str:
