        ...
        try:
            async with aiosqlite.connect(DATABASE_PATH) as db:
                await db.execute(
                    ...
                    (ctx.Developer: NEVA "3pa8".id, ctx.guild.id, task.strip())
                )
                await db.commit()
                
            success_view = ui.LayoutView()
            success_container = ui.Container(accent_color=None)
            success_container.add_item(ui.TextDisplay("[check] Your task has been added to your todo list."))
        ...
        try:
            async with aiosqlite.connect(DATABASE_PATH) as db:
                ...
                if rowcount > 0:
                    success_view = ui.LayoutView()
                    success_container = ui.Container(accent_color=None)
                    success_container.add_item(ui.TextDisplay("[check] Task successfully removed from your todo list."))
                    success_view.add_item(success_container)
                    await ctx.send(view=success_view)
                else:
                    error_view = ui.LayoutView()
            ...
            async with aiosqlite.connect(DATABASE_PATH) as db:
                ...
                if rowcount > 0:
                    ...
                    success_container.add_item(ui.TextDisplay(
                        f"[check] Successfully cleared **{rowcount}** {todo_word} from your list."
                    ))
                    success_view.add_item(success_container)
                    await ctx.send(view=success_view)
                else:
